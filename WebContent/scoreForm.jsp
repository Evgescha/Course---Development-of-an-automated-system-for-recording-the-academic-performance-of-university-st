...
<body>
	...
	<form action="scoreUpdate" method="post">
		<table border="1" cellpadding="5">
			 ...
			 <tr>
		    	...
		    	<td>
		    		<select name="studentId"  required>
			    		<c:forEach var="type" items="${students}">
				 			<option name="studentId" value="${type.id}">${type.name}</option>				    		
			    		</c:forEach>
					</select>
				</td>
           	</tr>
		    	...
		    	<td>
		    		<select name="subjectId"  required>
			    		<c:forEach var="type" items="${subjects}">
				 			<option name="subjectId" value="${type.id}">${type.name}</option>				    		
			    		</c:forEach>
					</select>
				</td>
           	</tr>
			<tr>
				<th>Оценка:</th>
				<td><input required type="number" name="score" size="45" value="<c:out value='${entity.score}' />" /></td>
			</tr>
			<tr>
				<th>Семестр:</th>
				<td><input required type="number" name="semestr" size="45" value="<c:out value='${entity.semestr}' />" /></td>
			</tr>
			<tr>
